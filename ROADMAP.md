...
### 🐞 To investigate
- ✅ **FIXED — Phantom conversation count** (`b81f887`, reported 2026-06-25) — root cause was NOT active-session detection: `restoreSession` rewrites a discovered `new` tile to `id:'resume:<sessionId>'`, and if that conversation was also open as a direct `resume:` tile, the two persisted entries collapsed to one id. `syncCockpit` (DOM map keyed by id) rendered one tile while the focus-chip count read `cp.tiles` directly → showed two ("2 cldctrl, 1 visible"). Fix: dedupe the restored tile set by id (the only producer of dup ids). Self-healing — the deduped set persists back on next change. Verified with a synthetic test. ⚠️ Live-confirm after restart+refresh that the count matches visible tiles.
...
- **CTRL as a default cockpit tile** *(user idea 2026-06-25)* — surface the mission-control (CTRL) agent as a first-class cockpit conversation, toggleable on/off, instead of (or in addition to) the right-rail dock. Behaviors: (1) **up by default when the cockpit is empty** (no other tiles) so there's always a place to start + mission-control is one glance away; (2) **fresh/blank on a real restart**, EXCEPT a very-recent reopen (app restart inside the ~8-min FRESH_MS window) keeps the existing CTRL conversation — reuse the persisted-session fresh-vs-resume logic we already have for tiles; (3) **seed memory of previous topics** — a fresh CTRL convo can be prefilled (confirm-before) with a short recap from the control workspace's existing `recaps/` (ties to #12 working-memory v0). **Architecture tailwind:** PTYs already live SERVER-SIDE in the named-terminal registry (multi-client views by id), so a CTRL *tile* can attach to the SAME control PTY as the dock — no double-spawn; the tile is just another view. **Decisions to make:** dock vs tile as the primary home (recommend: CTRL becomes a real tile `kind:'control'` attaching to the control terminal id; dock retires or becomes a quick-peek/"pop CTRL into a tile" affordance — avoid two confusingly-separate CTRL surfaces); how "toggle off" behaves (hide the view, keep the PTY); whether the empty-cockpit default is opt-out. Note: CTRL's fresh-on-restart policy is intentionally DIFFERENT from regular tiles' silent auto-resume (user wants CTRL to start clean unless very recent).
...
- ✅ **DONE — Figure lightbox: near-fullscreen zoom + gallery stepper** *(user-requested 2026-06-25 / 2026-06-30)*. The Stats image lightbox now shows ONE image at a time at ≈90vw/90vh (`object-fit: contain`) on a dimmed backdrop, with a `‹`/`›` on-screen stepper + ←/→ keys, a `i / N` counter, and Esc/✕/backdrop close — steps through all images in the clicked turn/bucket. (`web/stats.ts` `openLightbox`/`lbRender`/`lbStep`, `serve.ts` markup, `app.css`.) e2e-verified against a real 9-image turn (`e2e-image-gallery.mjs`). *Remaining: the gallery is reachable only from the Stats usage-timeline image markers — clicking an image **inline in a conversation** needs the "Embedded images in agent output" item below (terminals don't render images); once that lands, point those inline thumbnails at this same lightbox for a per-conversation gallery.*
...
- **Surface + attach Claude Code background agents** *(user idea 2026-06-30)* — cldctrl's session detection is 100% homegrown (markers + PID + JSONL **mtime** in `processes.ts`) and **never calls `claude agents`**, so it's blind to Claude Code's own agent registry — including `--bg` background agents and, crucially, their *actionable* status. `claude agents --json` (TTY-free, scriptable; `--all` = include completed, `--cwd` = filter by project) returns per-agent `kind` (background/interactive), `status` (idle/busy/waiting), `state` (e.g. `blocked`), and **`waitingFor: "permission prompt"`** — the one signal mtime can never give: *which of your N agents are parked needing a human, and why* (permission vs. a question vs. idle). Today you only find a blocked bg agent by stumbling into its terminal. **Tiers:** (1) **Discovery + "needs you" badge** — daemon polls `claude agents --json`, merges into the session view, flashes agents that are `blocked`/`waitingFor: permission` (reuses the existing cockpit attention/bell flash); also fold bg agents into `get_active_sessions` so the MCP tools see them. *Easy — all infra exists.* (2) **One-click attach for STOPPED sessions** — a row → cockpit resume tile already works (`serve.ts:719` spawns `claude --resume <sessionId>` in a node-pty tile); just wire agent-list rows to it. Caveat: `--resume` **refuses while the agent is still running as bg** (the exact error a user hit), so the bg flow is *stop → resume*, or show "running bg — attach in terminal." (3) **In-dashboard attach to a LIVE bg agent** — ⚠️ partly blocked: `claude agents` only attaches via its **interactive picker TUI** (no `--attach <id>` / headless attach-by-id flag exists as of CLI 2.1.197). Workarounds: host the `claude agents` TUI inside a cockpit PTY tile (xterm can render it; user drives the picker in-browser), or wait for the CLI to add a targeted-attach affordance. **Recommend building Tier 1 first** (high value, low risk).
- **Embedded images in agent output (inline + clickable → gallery)** *(user-flagged 2026-06-30)* — **NEEDED.** Conversation tiles are raw xterm terminals, so images the agent reads/creates/references **don't render at all** — you can't see a screenshot, diagram, or generated figure without leaving the cockpit. We need to (1) **detect image paths/refs in a conversation's output** (tool results that read/write image files, markdown `![]()`, pasted-image attachments in the JSONL) and (2) **surface them inline** — at minimum a small clickable thumbnail strip per tile (full inline rendering is constrained by xterm; an overlay/gutter beside the terminal is the likely shape). Clicking a thumbnail should **open the gallery lightbox we already shipped** (`web/stats.ts` `openLightbox`/`lbStep` — single image, `‹`/`›` + ←/→ stepping, counter, Esc) seeded with **all images in that conversation**, so you can step through a session's figures. This is the missing half that turns the Stats-only gallery into a true **per-conversation image gallery** — the thing the user actually asked for ("click an image listed in a conversation → popup viewer, step back/forward"). Data sources to mine: JSONL tool-use blocks (Read/Write on `.png/.jpg/.svg/...`), the same bucket-images path Stats already uses (`fetchBucketImages`), and bracketed image attachments. **Open question:** inline-in-terminal (sixel/iTerm img protocol — spotty in browser xterm) vs. a side/overlay thumbnail rail (more reliable) — lean to the rail.
